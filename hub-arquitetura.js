(function () {
  ...
  const stateKey = 'infrasec-deep-architecture';
  const deliverablesKey = 'infrasec-deliverables';
  ...
  function addDeliverable(item, report, score) {
    const list = read(deliverablesKey, []);
    const sourceId = `deep-architecture-${item.id}`;
    if (list.some((entry) => entry.sourceId === sourceId)) return;
    ...
    localStorage.setItem(deliverablesKey, JSON.stringify(list));
