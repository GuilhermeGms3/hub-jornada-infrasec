(function () {
  ...
  const adaptive = window.InfraSecAdaptive;
  if (!adaptive) return;
  ...
  const stateKey = adaptive.masteryKey;
  const read = () => {
  ...
  function startExam(domainId = domainSelect.value) {
    ...
    if (competency.level === 2) {
      window.InfraSecHub.navigateToPage(adaptive.domains[domainId].page);
      return;
    }
  ...
  function finishExam() {
    ...
    const correct = questions.filter((item, index) => answers[index] === item.correct).length;
    const score = correct * 25;
    const passed = score >= 75;
    const state = read();
    const domainState = state[activeDomain] || { history: [] };
    const previous = domainState[activeStage] || {};
    domainState[activeStage] = {
      passed: Boolean(previous.passed || passed),
      best: Math.max(Number(previous.best || 0), score),
      attempts: Number(previous.attempts || 0) + 1,
      updatedAt: new Date().toISOString()
    };
    domainState.history = [{ stage: activeStage, score, passed, createdAt: new Date().toISOString() }, ...(domainState.history || [])].slice(0, 20);
